...
var (
	ErrOutOfCapacity = errors.New("algokit: 超出最大容量限制")
	ErrEmptyQueue    = errors.New("algokit: 队列为空")
)
...
func (p *PriorityQueue[T]) Enqueue(t T) error {
	...
	for parent > 0 && p.comparator(p.data[node], p.data[parent]) < 0 {
...
func (p *PriorityQueue[T]) heapify(data []T, n, i int) {
	minPos := i
	for {
		if left := i * 2; left <= n && p.comparator(data[left], data[minPos]) < 0 {
			minPos = left
		}
		if right := i*2 + 1; right <= n && p.comparator(data[right], data[minPos]) < 0 {
			minPos = right
		}
		if minPos == i {
			break
